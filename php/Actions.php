...
Class Actions extends DBConnection{
    ...
    function register_user(){
        ...
        {
            $sql = "INSERT INTO user_list (username,contact,email,password) VALUES ('{$username}','{$PhoneNumber}','{$email}','{$password}')";
            $qry = $this->query($sql);
            if(!$qry){
                $resp['status'] = "failed";
        ...
        {
            $resp['status'] = "failed";
            $resp['msg'] = "Password and Confirm Password Not Same..!";
        }
        return json_encode($resp);
    }
    function logout(){
